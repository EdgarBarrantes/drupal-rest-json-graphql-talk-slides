...
<Notes>
  <div>
    - Primero vamos a introducir el tema hablando sobre la tendencia a
    desacoplar el frente del backend
  </div>{" "}
  <div>
    - Luego, uno por uno, vamos a describir que es REST API, JSON:API y GraphQL
    y sus implementaciones en Drupal
  </div>{" "}
  <div>
    - Mostraremos una aplicacion desarrollada para esta charla que interactua
    con cada uno de los APIs
  </div>{" "}
  <div>- Vamos a mostrar como interactuar con cada uno de estos APIs</div>{" "}
  <div>
    - Finalizaremos con algunos consejos sobre que no hacer, considerando
    tambien nuestra experiencia en proyectos Drupal desacoplados
  </div>{" "}
  <em
    style={{
      color: "black",
...
<Notes>
  ...
  <div>
    - No esta ligado a una base de datos, ni un lenguaje de programacion
    especifico.
  </div>
  <div>
    - Mas bien, esta soportado por el codigo existente y su informacion.
  </div>
  <div>
    - A diferencia de REST (un recurso por endpoint), mediante GraphQL se
    interactua con solo un endpoint y nuestro "query" es el que la informacion a
    retornar.
  </div>
...
<Notes>
  ...
  <div>
    - Un buen caso de uso para GraphQL es tener un equipo dev acostumbrado a
    interactuar con APIs GraphQL.
  </div>{" "}
  <div>
    - Que ademas tengan algun conocimiento de PHP para cumplir requerimientos de
    mutations.
  </div>{" "}
  <div>
    - Sin embargo, aun no estan estandarizadas todas las formas de interactuar
    con APIs de GraphQL.
  </div>{" "}
</Notes>
